package com.colormeter.reader.encode;
...
public class MachineCmd {
     ...
     * 获取第N条标样下第M条试样数据命令
     *
     * @param standarNum 标样的序号
     * @param sampleNum  试样的序号
     * @return
     */
    public static byte[] getNumSampleDataForNumStandardCmd(short standarNum, short sampleNum) {
        byte[] bytes = new byte[10];
        Arrays.fill(bytes, (byte) 0x00);
        bytes[0] = (byte) 0xbb;
        bytes[1] = 0x16;
        bytes[2] = 0x06;
        ByteUtil.putShort(bytes, standarNum, 3, ByteUtil.TYPE_LOW_FIRST);
     ...
     * 删除第N条标样下所有试样命令
     *
     * @param stanardNum 标样的序号
     * @return
     */
    public static byte[] deleteAllSampleForStandardNumCmd(short stanardNum) {
        byte[] bytes = new byte[10];
        Arrays.fill(bytes, (byte) 0x00);
        bytes[0] = (byte) 0xbb;
        bytes[1] = 0x16;
        bytes[2] = 0x07;
        ByteUtil.putShort(bytes, stanardNum, 3);
        bytes[8] = (byte) 0xff;
        return sumCheck(bytes);
    }
     ...
     * @return
     */
    public static byte[] deleteNumSampleDataForNumStandardCmd(short standarNum, short sampleNum) {
        byte[] bytes = new byte[10];
        Arrays.fill(bytes, (byte) 0x00);
        bytes[0] = (byte) 0xbb;
        bytes[1] = 0x16;
        bytes[2] = 0x08;
        ByteUtil.putShort(bytes, standarNum, 3);
    ...
    private static byte[] toStandardByte(StandardSampleDataBean.StandardDataBean bean) {
        ...
        bytes[4] = (byte) (bean.getStartWava() / 10);
        bytes[5] = bean.getWavaLength();
        bytes[6] = bean.getWavaCount();
     ...
     * @param colorSpace        颜色空间
     * @param colorDifference   色差公式
     * @param secondAngle       第二角度 （用于计算同色异谱）
     * @param secondLightSource 第二光源（用于计算同色异谱）
     * @return
     */
    public static byte[] setDeviceDisplayParamCmd(int lightSource, int angle,
                                                  int measureMode, int colorSpace,
     ...
     * 设置电源管理时间命令
     *
     * @param autoOffTime   关机时间
     * @param backLightTime 背光时间
     * @return
     */
